...
const LIFESPAN_FALLBACK = {
    looted: "ephemeral",
    "boss-slain": "persistent",
    collapsed: "succession-seed",
};
export function lifespanFor(tables, kind) {
...
function stampForRoom(roomId, kind) {
    const areaId = getRoomArea(roomId);
    const tables = areaId ? (getAreaState(areaId)?.sixAxis || {}) : {};
    tapestry.consequence.stamp(roomId, kind, lifespanFor(tables, kind));
}
// Registered at module load. Guard everything; never throw into the engine loop.
export function registerConsequenceHooks() {
    tapestry.events.on("mob.death", function (evt) {
        try {
            ...
            const remaining = tapestry.world.getEntitiesInRoom(roomId, "npc");
