...
export function parseSixAxisTable(raw) {
    if (!raw || typeof raw !== "object") {
    ...
    if (axis === "DEGREE") {
        ...
        for (const k in subs) {
            ...
            if (k === "state_overrides") {
                ...
                t.stateOverrides = nativeStringMap(raw.subtables.state_overrides);
            }
            else {
        ...
        for (let ti = 0; ti < tierNames.length; ti++) {
            ...
            const list = nativeArray(tiers[tier]);
            for (let i = 0; i < list.length; i++) {
                const c = list[i];
                t.consequences.push({
                    id: String(c.id),
                    effect: String(c.effect || ""),
                    lifespan: String(c.lifespan || "ephemeral"),
                    tier: tier,
                });
            }
        }
