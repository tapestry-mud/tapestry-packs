...
const data = engineData.loadYaml("data/master-balance.yml");
...
export function interpolateNumeric(anchors, values, level) {
...
function nearestAnchor(anchors, level) {
    const L = clampLevel(level);
    let best = anchors[0];
...
export function rarityModifier(rarity) {
    const r = data.rarity || {};
    return typeof r[rarity] === "number" ? r[rarity] : 0;
}
export function statsFor(kind, level, rng) {
    const L = clampLevel(level);
    if (kind === "mob") {
        ...
        const flee = interpolateNumeric(a, data.mob.flee_threshold.map((f) => Math.round(f * 100)), L) / 100;
...
export function bossSwellDials(rank) {
 ...
 */
export function mobHpFormula(level) {
    return statsFor("mob", level, () => 0.5).hp;
}
