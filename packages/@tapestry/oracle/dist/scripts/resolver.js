...
// ---------------------------------------------------------------------------
export function rollEntry(entries, rng) {
...
// ---------------------------------------------------------------------------
export function shouldReuse(existingCount, rng) {
    if (existingCount <= 0) {
        return false;
    }
...
// ---------------------------------------------------------------------------
function table(areaId, kind) {
    const t = tapestry.oracle.table(areaId + ":" + kind);
    if (!t || !t.entries) {
        return [];
    }
    return t.entries;
...
// ---------------------------------------------------------------------------
function rollFormula(notation, rng) {
    const m = /^(\d+)d(\d+)$/.exec(notation);
    if (!m) {
        return parseInt(notation, 10) || 1;
    }
...
// ---------------------------------------------------------------------------
export function mintMobInstance(areaId, level, rng) {
...
// Returns null if the table is empty.
// ---------------------------------------------------------------------------
export function mintBossInstance(areaId, level, rng) {
    ...
    return {
        ...
        name: type.name,
        desc: type.desc,
        maxHp: rollFormula(stats.hp, rng),
        damage: stats.damage,
        noReroll: true,
    };
}
...
// ---------------------------------------------------------------------------
export function rngFor(areaSeed, key) {
