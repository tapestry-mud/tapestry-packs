...
export function fillTables(
    ...
    call(recommend, "fill_places", vars, (placesRaw) => {
        ...
        // items fires after mobs or boss completes (keeping in-flight <= 2 once either resolves).
...
function fillProse(
    ...
    call(recommend, "fill_prose_openers", proseVars, (raw) => { pushLines(proseEntries, raw, "opener"); done(); });
    call(recommend, "fill_prose_details", proseVars, (raw) => { pushLines(proseEntries, raw, "detail"); done(); });
    // atmosphere fires after one of the above completes (in-flight stays <= 2).
