...
function scarLineFor(roomId: string): string {
    const entries: any[] = (tapestry as any).consequence.list(roomId) || [];
    if (entries.length === 0) {
        return "";
    }
    const kinds: string[] = [];
...
export function registerRevisitHooks(): void {
    (tapestry as any).events.on("player.direction.moved", function (evt: any): void {
        try {
            ...
            if (line !== "") {
                (tapestry as any).world.send(entityId, line + "\r\n");
            }
        } catch (_err) {
