...
import type { OracleEntry } from "./oracle-tables.js"; // type-only: erased at compile, no runtime engine pull
...
export function parsePipeLines(raw: string | null, defaultBalanceRef: string, isItem: boolean): OracleEntry[] {
    ...
    for (const rawLine of raw.split("\n")) {
        if (isPreamble(rawLine)) { continue; }
...
export function pushLines(out: OracleEntry[], raw: string | null, kind: string): void {
    if (!raw) { return; }
    let i = 0;
    for (const rawLine of raw.split("\n")) {
        if (isPreamble(rawLine)) { continue; }
        const t = capFragment(cleanLine(rawLine));
        if (t.length === 0 || !/[a-z0-9]/i.test(t)) { continue; }
