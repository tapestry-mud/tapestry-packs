...
- The recipe table is a module-private `_recipes` map keyed by scoped recipe id
  (e.g. `tapestry-tinkers:campfire-portable`). It is initialized at module load
  time and not exposed directly; callers use the exported functions.
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:3)

- `addRecipe(recipe)` validates that `recipe.id` and `recipe.output` are
  present, then stores the record. It is a native module export (re-exported from
  `scripts/index.ts`) so other packs can import it and contribute recipes at their
  own load time, over a declared dependency edge.
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:28-34;
  packages/@tapestry/tinkers/scripts/index.ts:1)
...
- A recipe object has four fields: `id` (scoped string), `name` (optional
  ...
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:22-27)
...
- `findRecipe(nameOrId)` resolves a recipe by exact scoped id, exact short id
  ...
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:50-75)
...
- Matching is tolerant: input is lowercased, surrounding quotes are stripped,
  ...
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:11-16)
...
- A `'recipe'` arg type is registered with the engine arg resolver. When a
  ...
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:79-89)

- `findRecipe` and `displayName` are native exports; the sibling scripts (craft.ts,
  recipes.ts) reach them via a relative import
  (`import { findRecipe, displayName } from "../recipes-table.js"`), replacing the
  former self-referential `tapestry.packs.require('@tapestry/tinkers')`.
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:36,50;
  packages/@tapestry/tinkers/scripts/commands/craft.ts:2)
...
- Two bootstrap recipes are registered at load time:
  ...
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:95-107)
...
- Registered in the `inventory` category, roles: `['player']`. Takes one
  ...
  handler runs). (packages/@tapestry/tinkers/scripts/commands/craft.ts:2-12)
...
- Recipe knowledge gate: if `benchLevelRequired > 0`, the handler reads the
  ...
  (packages/@tapestry/tinkers/scripts/commands/craft.ts:25-30)
...
- Bench gate: if `benchLevelRequired > 0`, the handler scans the player's
  ...
  (packages/@tapestry/tinkers/scripts/commands/craft.ts:35-61)
...
- Validate-all-before-consume: the handler iterates every input entry and
  ...
  (packages/@tapestry/tinkers/scripts/commands/craft.ts:64-98)
...
- Input matching is by `material` or by `id` (template id), not both on the
  ...
  (packages/@tapestry/tinkers/scripts/commands/craft.ts:75-82)
...
- On success, consumed items are destroyed via `tapestry.inventory.destroy`
  ...
  (packages/@tapestry/tinkers/scripts/commands/craft.ts:102-113)

- The actor sees "You craft <output.name>." and the room sees "<actor.name>
  crafts something." (packages/@tapestry/tinkers/scripts/commands/craft.ts:113-114)

### `recipes` command

- Registered in the `inventory` category, roles: `['player']`. Takes one
  optional `recipe` arg. (packages/@tapestry/tinkers/scripts/commands/recipes.ts:2-11)
...
- List view (no arg): reads `known_recipes` from the player entity. If empty,
  ...
  (packages/@tapestry/tinkers/scripts/commands/recipes.ts:13-15;
  packages/@tapestry/tinkers/scripts/commands/recipes.ts:46-65)
...
- Detail view (`recipes <name>`): resolves the arg to a recipe and prints its
  ...
  (packages/@tapestry/tinkers/scripts/commands/recipes.ts:18-43)
...
- The `recipes` command does not gate on known_recipes when showing details --
  ...
  (packages/@tapestry/tinkers/scripts/commands/recipes.ts:18-22)

### `copy` command

- Registered in the `inventory` category, roles: `['player']`. Takes one
  required `findable` arg (an item the player can locate by standard find
  rules). (packages/@tapestry/tinkers/scripts/commands/copy.ts:2-8)
...
- Reads `teaches_recipe` from the target item. If the property is absent,
  ...
  (packages/@tapestry/tinkers/scripts/commands/copy.ts:11-15)
...
- Duplicate guard: if the recipe id is already in the player's `known_recipes`
  ...
  (packages/@tapestry/tinkers/scripts/commands/copy.ts:21-23)
...
- On success, appends the recipe id to `known_recipes` via
  ...
  (packages/@tapestry/tinkers/scripts/commands/copy.ts:26-40)

- The room sees "<actor.name> studies <item.name>, then sets the spent
  schematic aside." (packages/@tapestry/tinkers/scripts/commands/copy.ts:35)
...
- The campfire recipe's output (`tapestry-cooking:campfire-portable`) is owned
  ...
  packages/@tapestry/tinkers/scripts/recipes-table.ts:103-107)

- `addRecipe` is a native export for external packs to import and register
  additional recipes into the Tinkers table without forking the pack.
  (packages/@tapestry/tinkers/scripts/recipes-table.ts:28;
  packages/@tapestry/tinkers/scripts/index.ts:1)
