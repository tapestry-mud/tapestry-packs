...
Current version: 0.1.3. Requires `@tapestry/core ^0.1.4`; optionally integrates
with `@tapestry/survival ^0.1.1`. (packages/@tapestry/cooking/pack.yaml:1-13)
...
- The `cook` command is registered in the `inventory` category and is available
  ...
  from the actor's inventory. (packages/@tapestry/cooking/scripts/commands/cook.js:1-8)
...
- Before any tool search, the command verifies the target item carries both the
  ...
  message to the actor. (packages/@tapestry/cooking/scripts/commands/cook.js:12-22)
...
- Tool resolution checks the actor's inventory first; if no `can_cook` item is
  ...
  (packages/@tapestry/cooking/scripts/commands/cook.js:26-45)

- If no cooking tool is found in either inventory or room, the command aborts
  with "You need something to cook with." (packages/@tapestry/cooking/scripts/commands/cook.js:47-50)
...
- After a tool is found, the command checks whether the tool itself carries
  ...
  (packages/@tapestry/cooking/scripts/commands/cook.js:52-68)
...
- The raw ingredient is consumed via `tapestry.consumables.consume`. Failure
  ...
  `tapestry.items.spawnToInventory`. (packages/@tapestry/cooking/scripts/commands/cook.js:70-80)
...
- Feedback text is taken from the tool's `cook_text` property if present, with
  ...
  something." is always broadcast. (packages/@tapestry/cooking/scripts/commands/cook.js:82-88)
...
- If the cooking tool carries `destroy_on_empty`, the tool itself is consumed
  ...
  are left untouched and remain reusable. (packages/@tapestry/cooking/scripts/commands/cook.js:92-94)
...
- cook-buff.js registers a listener on the `item.consumed` event. It ignores any
  consumption that did not use the `eat` method and ignores items that do not
  carry the `cooked` property. (packages/@tapestry/cooking/scripts/cook-buff.js:1-4)

- When a cooked item is eaten and the `@tapestry/survival` pack exposes
  `applyWellFedBuff`, that function is called with the actor's entity id and a
  duration value of `3000`. The call is guarded by `tapestry.packs.has`, so the
  survival pack remains a soft optional dependency -- cooking functions fully
  without it. (packages/@tapestry/cooking/scripts/cook-buff.js:5-8)
