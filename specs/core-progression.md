...
- Two tracks are registered at startup: `combat` and `magic`, each with `max_level: 50`
  ...
  advance from the given level. (packages/@tapestry/core/scripts/progression/progression.js:8-62)

- Both tracks carry `death_penalty: 0.1`, meaning 10% of within-level progress is lost
  on death. (packages/@tapestry/core/scripts/progression/progression.js:15; packages/@tapestry/core/scripts/progression/progression.js:43)
...
- The combat track's `on_level_up` callback grants +1 to two randomly chosen stats from
  ...
  and +2 max_movement per level. (packages/@tapestry/core/scripts/progression/progression.js:16-35)
...
- The magic track's `on_level_up` callback draws from a weighted pool
  ...
  (packages/@tapestry/core/scripts/progression/progression.js:45-61)
...
- On level-up, both tracks send a notification via `tapestry.notifications.enqueue` at
  ...
  (packages/@tapestry/core/scripts/progression/progression.js:33-34; packages/@tapestry/core/scripts/progression/progression.js:59-60)

- The `tree` command reads a class's `track` property to resolve which progression track
  controls class level for the ability path display. (packages/@tapestry/core/scripts/commands/tree.js:14)

### Kill XP

- XP is awarded on the `combat.kill` event. No XP is given for killing players
  (`victim.type === "player"` guard). (packages/@tapestry/core/scripts/progression/progression.js:73-83)
...
- Base XP for a kill uses the mob's `xp_value` property if set; otherwise falls back to
  ...
  (packages/@tapestry/core/scripts/progression/progression.js:66-69; packages/@tapestry/core/scripts/progression/progression.js:86-87)
...
- All player entities actively in combat with the killed mob are collected via
  ...
  sole recipient. (packages/@tapestry/core/scripts/progression/progression.js:90-94)
...
- Same-room group members of any player combatant who share the victim's room are also
  ...
  in the room even if they did not strike the mob.
  (packages/@tapestry/core/scripts/progression/progression.js:109-123)
...
- Each eligible player receives XP scaled by `tapestry.progression.calculateMobXp` using
  ...
  then floored. (packages/@tapestry/core/scripts/progression/progression.js:125-135)

- XP is granted to the `combat` track only via `tapestry.progression.grant(..., "combat", "kill")`.
  (packages/@tapestry/core/scripts/progression/progression.js:134)
...
- When the source is `"kill"`, a `progression.xp.gained` listener sends the message
  ...
  (packages/@tapestry/core/scripts/progression/progression.js:141-151)
...
- On `player.death`, every registered track with `death_penalty > 0` deducts XP. The
  ...
  (packages/@tapestry/core/scripts/progression/progression.js:155-179)

- The loss message uses the `<death>` tag: `You lose N experience.`
  (packages/@tapestry/core/scripts/progression/progression.js:173-175)
...
- `train` (no argument) displays a panel titled "Your Attributes" listing all six base
  ...
  (packages/@tapestry/core/scripts/commands/train.js:18-82)
...
- `train [stat]` accepts the stat name or its abbreviation/prefix and resolves it via a
  ...
  (packages/@tapestry/core/scripts/commands/train.js:1-14)
...
- A successful train calls `tapestry.training.trainStat(entityId, statName)` and returns
  ...
  (packages/@tapestry/core/scripts/commands/train.js:91-110)
...
- `practice` (alias `prac`) with no argument lists all learned abilities with proficiency
  ...
  (packages/@tapestry/core/scripts/commands/practice.js:8-46; packages/@tapestry/core/scripts/commands/practice.js:48-95)
...
- `practice [ability]` requires a trainer NPC in the same room. On success the trainer
  ...
  ability ID. (packages/@tapestry/core/scripts/commands/practice.js:97-122)
...
- `tree` renders the class ability path split into "Learned" and "Upcoming" sections.
  ...
  (packages/@tapestry/core/scripts/commands/tree.js:1-115)

- `tree skills` or `tree spells` filters the display to the given category.
  (packages/@tapestry/core/scripts/commands/tree.js:117-128)
...
- `score` renders a full character sheet panel containing: character name, all six
  ...
  through level). (packages/@tapestry/core/scripts/commands/score.js:14-221)
...
- A `Response.Char.Score` GMCP message is sent before rendering, containing the full data
  ...
  `xpTracks` array. (packages/@tapestry/core/scripts/commands/score.js:48-73)
...
- When the terminal width is known and less than 62 columns, a compact single-column
  ...
  the full layout. (packages/@tapestry/core/scripts/commands/score.js:80-81)
...
- The per-track progress percentage is computed as
  ...
  At max level it shows 100%. (packages/@tapestry/core/scripts/commands/score.js:98-105)
...
- `affects` (aliases `aff`, `effects`) lists all active effects from
  ...
  (packages/@tapestry/core/scripts/commands/affects.js:1-61)

- If no effects are active, a panel with the message "You are not affected by anything."
  is shown. (packages/@tapestry/core/scripts/commands/affects.js:11-20)

### Rest States

- The three rest states managed by `tapestry.rest` are `awake`, `resting`, and `sleeping`.
  (packages/@tapestry/core/scripts/commands/rest.js:8-9; packages/@tapestry/core/scripts/commands/sleep.js:9; packages/@tapestry/core/scripts/commands/wake.js:8)
...
- `rest` blocks if the actor is already resting or sleeping, or is in combat.
  ...
  (packages/@tapestry/core/scripts/commands/rest.js:9-25)
...
- `sleep` blocks if the actor is already sleeping, or is in combat. Setting state to
  ...
  (packages/@tapestry/core/scripts/commands/sleep.js:9-25)
...
- `wake` (alias `stand`) can be called from any non-awake state without a combat check.
  ...
  (packages/@tapestry/core/scripts/commands/wake.js:8-21)
...
- If combat begins while an entity is asleep or resting, the engine fires
  ...
  (packages/@tapestry/core/scripts/rest.js:1-10)
...
- `quests` (alias `journal`) displays active quests from `tapestry.quests.getState`,
  ...
  (packages/@tapestry/core/scripts/commands/quests.js:27-70)
...
- The quest state object exposes `state.active`; there is no `state.completed` field
  ...
  (packages/@tapestry/core/scripts/commands/quests.js:28-29)
