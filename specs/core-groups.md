...
- Group IDs are generated with `'grp_' + Date.now().toString(36) + '_' + random`
  ...
  invitation, not when an invitation is sent. (packages/@tapestry/core/scripts/commands/groups.ts:1-2)
- When the first `group accept` fires, if the inviter has no existing group, a
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:393-398)
- Each member tracks three world properties: `group_id`, `group_leader`, and
  `group_join_time` (set to `Date.now()` at join time).
  (packages/@tapestry/core/scripts/commands/groups.ts:47-50)
...
- An invitation stores `group_invite_from` and `group_invite_expires` on the
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:364-365)
- The inviter need not already be in a group at invitation time; the group is
  created on accept. (packages/@tapestry/core/scripts/commands/groups.ts:393-396)
- `group decline` clears both invite properties and notifies the inviter if
  still online. (packages/@tapestry/core/scripts/commands/groups.ts:412-425)
...
- The `group` command (alias `gr`) dispatches on a `subcommand` keyword. With no
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:315-337)
- `group kick [player]` is leader-only and requires the target to share the
  ...
  notified. (packages/@tapestry/core/scripts/commands/groups.ts:473-511)
...
- The `group` command with no arguments renders a panel with a title row showing
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:574-606)
- Level is read from `tapestry.progression.getLevel(memberId, 'combat')`.
  (packages/@tapestry/core/scripts/commands/groups.ts:587)
...
- `gtell` (alias `gt`) sends a message to all online members of the actor's
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:610-632)
- Messages are formatted as `<group>[Group] Name: "text"</group>`.
  (packages/@tapestry/core/scripts/commands/groups.ts:629)
- `sendToGroup` delivers to every member including the sender.
  (packages/@tapestry/core/scripts/commands/groups.ts:58-63)
...
- On `combat.kill`, if the killer is in a group, the victim's gold is split
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:636-668)
- The split only fires when there are at least two recipients (killer + at least
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:651-653)
- Integer remainder goes to the killer. All recipients see the message "amount
  gold coins are divided among the group."
  (packages/@tapestry/core/scripts/commands/groups.ts:657-667)
- `getSameRoomGroupMembers` excludes the actor from the member list, so the
  killer is added separately as `recipients[0]`. It is a native module export so
  `progression.ts` can import it for shared-room XP distribution, replacing the
  implicit shared-realm global the two files used to share.
  (packages/@tapestry/core/scripts/commands/groups.ts:36-44, 641)
...
- `follow [player]` sets `following` on the actor to the target's entity ID.
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:155-178)
- `follow stop` clears `following` and publishes `follow.ended` with
  `reason: 'command'`. (packages/@tapestry/core/scripts/commands/groups.ts:128-147)
- A player cannot follow themselves.
  (packages/@tapestry/core/scripts/commands/groups.ts:150-153)
...
- On `player.direction.moved`, all online players whose `following` property
  ...
  same direction. (packages/@tapestry/core/scripts/commands/groups.ts:219-259)
- A follower in a rest state (`resting` or `sleeping`) is silently skipped.
  (packages/@tapestry/core/scripts/commands/groups.ts:239-240)
- A follower in combat receives the message "You cannot follow while in combat."
  and is skipped. (packages/@tapestry/core/scripts/commands/groups.ts:242-245)
- On successful move, the follower receives a room description, disposition is
  ...
  rooms. (packages/@tapestry/core/scripts/commands/groups.ts:250-258)
...
- `nofollow` is a toggle. First use sets `no_follow = true` and immediately
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:195-214)
- Second use clears `no_follow` and the player is again accepting followers.
  (packages/@tapestry/core/scripts/commands/groups.ts:192-194)
- Note: the help file (nofollow.yaml) states nofollow "only blocks new follow
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:196-213,
...
- On `player.logout`, `no_follow` is cleared, `clearFollowState` is called, and
  ...
  (with a no-op `send`). (packages/@tapestry/core/scripts/commands/groups.ts:289-297)
- On `player.death`, only `clearFollowState` is called; the player is not
  removed from their group. (packages/@tapestry/core/scripts/commands/groups.ts:299-304)
- On `player.teleported`, only `clearFollowState` is called; the player is not
  removed from their group. (packages/@tapestry/core/scripts/commands/groups.ts:306-311)
- `clearFollowState` clears the entity's own `following` property and also
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:262-287)

### Events published

- `group.created` -- published when the first accept creates a new group.
  (packages/@tapestry/core/scripts/commands/groups.ts:405)
- `group.member.joined` -- published on every successful accept.
  (packages/@tapestry/core/scripts/commands/groups.ts:407-409)
- `group.member.left` -- published on leave (reason: 'leave').
  (packages/@tapestry/core/scripts/commands/groups.ts:442-444)
- `group.member.kicked` -- published on kick.
  (packages/@tapestry/core/scripts/commands/groups.ts:508-510)
- `group.member.promoted` -- published on manual promote and on auto-promote
  after leader departure. (packages/@tapestry/core/scripts/commands/groups.ts:548-550, 461-464)
- `group.disbanded` -- published on disband and on leader-leave when no
  successor can be found.
  (packages/@tapestry/core/scripts/commands/groups.ts:464, 571)
- `follow.started` -- published when a follow begins, carrying `followerId` and
  `leaderId` only (no `reason` field).
  (packages/@tapestry/core/scripts/commands/groups.ts:175-178)
- `follow.ended` -- published when a follow ends, carrying `followerId`, `leaderId`,
  ...
  (packages/@tapestry/core/scripts/commands/groups.ts:142-147;
  packages/@tapestry/core/scripts/commands/groups.ts:206-211;
  packages/@tapestry/core/scripts/commands/groups.ts:266-270;
  packages/@tapestry/core/scripts/commands/groups.ts:280-284)
...
- The `clan` command is a communication-only channel; there are no subcommands
  ...
  (packages/@tapestry/core/scripts/commands/clan.ts:1-40)
...
- Messages are formatted as `<clan>[Clan] Name: "text"</clan>` and also trigger
  ...
  (packages/@tapestry/core/scripts/commands/clan.ts:35-36)
