...
- `say` is registered with alias `'`, category `communication`, and roles
  `player` and `mob`. (packages/@tapestry/core/scripts/commands/say.js:2-6)
- The actor sees `You say "<highlight>...</highlight>"` and all other players in
  the same room see `<Name> says "<highlight>...</highlight>"` via
  `tapestry.world.sendToRoomExcept`. (packages/@tapestry/core/scripts/commands/say.js:12-17)
- `say` publishes two events: `communication.message` with `channel: "say"` and
  ...
  (packages/@tapestry/core/scripts/commands/say.js:19-33)
- The `communication.message` listener in `communication-gmcp.js` resolves
  ...
  sender. (packages/@tapestry/core/scripts/core/communication-gmcp.js:5-31)
- The command registration also carries `gmcp: { channel: 'say', prependSender: false }`;
  ...
  (packages/@tapestry/core/scripts/commands/say.js:7)

### emote

- `emote` is registered with alias `:`, category `communication`, and roles
  `player` and `mob`. (packages/@tapestry/core/scripts/commands/emote.js:2-6)
- The actor is blocked if the entity property `noemote` is truthy; they receive
  `You cannot emote right now.` (packages/@tapestry/core/scripts/commands/emote.js:12-15)
- Both the actor and all other room occupants see `<Name> <message>` -- no
  quotation marks are added. (packages/@tapestry/core/scripts/commands/emote.js:17-22)
- `emote` publishes `communication.message` with `channel: "emote"` and
  `text` set to the fully-prefixed string `<Name> <message>`.
  (packages/@tapestry/core/scripts/commands/emote.js:24-32)
- `communication-gmcp.js` routes emote the same as say: room-scoped recipient
  list, sender excluded. (packages/@tapestry/core/scripts/core/communication-gmcp.js:5-9)
- The command registration carries `gmcp: { channel: 'emote', prependSender: false }`;
  ...
  (packages/@tapestry/core/scripts/commands/emote.js:7)
...
- Validates that the stored player ID is still online via
  ...
  longer online.` (packages/@tapestry/core/scripts/commands/reply.js:19-31)
- Blocked if the sender has `notell`; error: `You cannot send tells right now.`
  (packages/@tapestry/core/scripts/commands/reply.js:33-36)
- On success, sends the same tell formatting and GMCP event as `tell`, and
  updates `last_tell_from` / `last_tell_to` on both parties.
  (packages/@tapestry/core/scripts/commands/reply.js:38-43)
- Note: `reply` does not check `nochannels` on the sender, unlike `tell`.
  (packages/@tapestry/core/scripts/commands/reply.js:33-36 vs packages/@tapestry/core/scripts/commands/tell.js:20-23)

### gossip

- `gossip` is registered with no alias, category `social`, roles `player` and
  `mob`. (packages/@tapestry/core/scripts/commands/gossip.js:2-5)
- Blocked if the actor has `nochannels`; error: `You cannot use channels right
  now.` (packages/@tapestry/core/scripts/commands/gossip.js:12-15)
...
- The command description states "Shout a message to the entire area" but the
  ...
  (packages/@tapestry/core/scripts/commands/yell.js:3,14)
- A `Comm.Channel` GMCP event with `{ channel: 'yell', sender, text }` (using
  ...
  (packages/@tapestry/core/scripts/commands/yell.js:17-19)
- `yell` has no `nochannels` gate; no mute property check is performed.
  (packages/@tapestry/core/scripts/commands/yell.js:1-21)
...
- `immtalk` is registered with alias `;`, category `social`, and `admin: true`;
  ...
  flag. (packages/@tapestry/core/scripts/commands/immtalk.js:2-7)
- Blocked if the actor has `nochannels`; error: `You cannot use channels right
  now.` (packages/@tapestry/core/scripts/commands/immtalk.js:11-14)
- Delivery iterates all online players and sends only to those where
  ...
  (packages/@tapestry/core/scripts/commands/immtalk.js:17-26)
- Message format: `<imm>[Imm] <Name>: "<message>"</imm>`.
  (packages/@tapestry/core/scripts/commands/immtalk.js:22)
- A `Comm.Channel` GMCP event with `{ channel: 'imm', sender, text }` is sent
  to each admin recipient. (packages/@tapestry/core/scripts/commands/immtalk.js:23)
...
- Listens on `communication.message`. For channels `say` and `emote`, recipients
  ...
  online players. (packages/@tapestry/core/scripts/core/communication-gmcp.js:4-16)
- Sends `Comm.Channel` with payload `{ channel, sender, senderId, source, text }`
  ...
  (packages/@tapestry/core/scripts/core/communication-gmcp.js:18-31)
- `tell` and `reply` do not publish `communication.message`; they call
  ...
  involved for those channels. (packages/@tapestry/core/scripts/commands/tell.js:32,
  packages/@tapestry/core/scripts/commands/reply.js:40)
- `gossip`, `yell`, and `immtalk` similarly bypass the event listener and call
  `tapestry.gmcp.send` inline. (packages/@tapestry/core/scripts/commands/gossip.js:23-27,
  packages/@tapestry/core/scripts/commands/yell.js:17-19,
  packages/@tapestry/core/scripts/commands/immtalk.js:23)
